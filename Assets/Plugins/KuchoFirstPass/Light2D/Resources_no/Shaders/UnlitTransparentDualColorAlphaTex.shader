...
Shader "Light2D/Unilt Transparent Dual Color AlphaTex" {
Properties {
	_MainTex ("Base (RGB) Trans (A)", 2D) = "white" {}
	_AlphaTex ("Alpha Tex", 2D) = "white" {} // KUCHO HACK
	_BlurDistance ("Blur Distance", Float) = 4 // blur distance in pixels
	_a ("a", Float) = 4 // borrame
}
	...
	Pass {  
		CGPROGRAM
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				half4 col = sum / 9.0;
			#else
				fixed4 col; // KUCHO HACK
				col.rgb = tex2D(_MainTex, i.texcoord).rgb; // KUCHO HACK
			#endif
				col.a = tex2D(_AlphaTex, i.texcoord).a; // KUCHO HACK
