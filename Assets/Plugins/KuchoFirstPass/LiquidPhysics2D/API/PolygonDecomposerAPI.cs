...
public static class PolygonDecomposerAPI {
	...
	**/
	public static List<List<Vector2>> DecomposePolygon(List<Vector2> vertices) {
		...
		int verticesCount = vertices.Count; // Optimizacion , leert .Count es el doble de lento!
		...
		for (int i = 0; i < verticesCount; ++i) {
			if (Reflex(i, vertices)) {
				...
				if (lowerIndex == (upperIndex + 1) % verticesCount) {
		...
		if (verticesCount > 8) { //MaxPolygonVertices
			lowerPoly = Copy(0, verticesCount / 2, vertices);
			upperPoly = Copy(verticesCount / 2, 0, vertices);
			list.AddRange(DecomposePolygon(lowerPoly));
			list.AddRange(DecomposePolygon(upperPoly));
		}
		else
	...
	private static bool CanSee(int i, int j, List<Vector2> vertices) {
		...
		for (int k = 0; k < verticesCount; ++k) {
			if ((k + 1) % verticesCount == i || k == i || (k + 1) % verticesCount == j || k == j)
