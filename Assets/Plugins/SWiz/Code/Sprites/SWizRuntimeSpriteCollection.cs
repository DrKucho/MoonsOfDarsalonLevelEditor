...
namespace SWizRuntime
{
	...
	{
		public static SWizSpriteCollectionData CreateFromTexture(Texture texture, SWizSpriteCollectionSize size, Rect region, Vector2 anchor)
		{
		...
		public static SWizSpriteCollectionData CreateFromTexture(
			Texture texture,
			SWizSpriteCollectionSize size,
			...
			bool[] rotated)
		{
			...
			SWizSpriteCollectionData sc = go.AddComponent<SWizSpriteCollectionData>();
			sc.Transient = true;
			sc.version = SWizSpriteCollectionData.CURRENT_VERSION;
			
			sc.invOrthoSize = 1.0f / size.OrthoSize;
			sc.halfTargetHeight = size.TargetHeight * 0.5f;
			sc.premultipliedAlpha = false;
			
			string shaderName = "SWiz/BlendVertexColor";
...
#if UNITY_EDITOR
			{
				...
				if (assetPath.ToLower().IndexOf("/resources/") == -1) {
					UnityEditor.EditorUtility.DisplayDialog("SWizRuntimeSpriteCollection Error",
						"The SWiz/BlendVertexColor shader needs to be in a resources folder for this to work.\n\n" +
						"Create a subdirectory named 'resources' where the shaders are, and move the BlendVertexColor shader into this directory.\n\n"+
						"eg. SpriteWizard/SWiz/Shaders/Resources/BlendVertexColor\n\n" +
			...
			// Generate geometry
			sc.spriteDefinitions = new SWizSpriteDefinition[regions.Length];
		...
		static SWizSpriteDefinition CreateDefinitionForRegionInTexture(string name, Vector2 textureDimensions, float scale, Rect uvRegion, Rect trimRect, Vector2 anchor, bool rotated)
		{
			...
			var def = new SWizSpriteDefinition();
			def.flipped = rotated ? SWizSpriteDefinition.FlipMode.TPackerCW : SWizSpriteDefinition.FlipMode.None;
			def.extractRegion = false;
			def.name = name;
			def.colliderType = SWizSpriteDefinition.ColliderType.Unset;
		...
		public static SWizSpriteCollectionData CreateFromTexturePacker( SWizSpriteCollectionSize spriteCollectionSize, string texturePackerFileContents, Texture texture ) {
