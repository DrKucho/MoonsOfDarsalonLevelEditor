...
//    public static void SetSize(int index) // antigua anterior a cuando añadieron Scale
//    {
...
//    }
    ...
    public static void SetSize(Vector2Int size, bool forceOpen){
        SetSize(size.x, size.y, forceOpen);
    }
    public static void SetSize(int width, int height , bool forceOpen){
        Vector2Int gameViewSize = GetCurrentSize();
        bool gameViewWindowIsOpened = true;
        if (gameViewSize.x == 640 && gameViewSize.y == 480) // por alguna razon devuelve estos valores cuando la ventana esta oculta, cerrada
            gameViewWindowIsOpened = false;
       
        if (forceOpen || gameViewWindowIsOpened)
        {
            if (gameViewSize.x != width || gameViewSize.y != height)
            {
                ...
                {
                    AddCustomSize(GameViewSizeType.FixedResolution, groupType, width, height, "");
                    index = FindSize(groupType, width, height);
                    SetSize(index);
                } 
            }
        }
    }
    public static Vector2Int GetCurrentSize() // movido a UIThing, habia dejado de funcionar de todas formas pero por si puedo arreglarlo en un futuro lo muevo alli
    {
        System.Type T= System.Type.GetType("UnityEditor.GameView,UnityEditor");
        System.Reflection.MethodInfo GetSizeOfMainGameView = T.GetMethod("GetSizeOfMainGameView",System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);
        System.Object Res = GetSizeOfMainGameView.Invoke(null,null); // no se por que me devuelve un Object... ?
        Vector2 resv2 = (Vector2)Res;// pero asi estbaa en javascript... asi que no lo toco, casteo aqui y fuera
        Vector2Int res = new Vector2Int( (int)resv2.x, (int)resv2.y);
        if (!Application.isPlaying)
            Debug.Log("GAME VIEW SIZE = " + res);
        return res;
    }
}
