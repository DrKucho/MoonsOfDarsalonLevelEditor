...
Shader "_Kucho/Sprites-Default - HSVCD-TintAdd v2"
{
     Properties
     {
         ...
         _AdditiveColor ("_AdditiveColor", Color) = (1,1,1,1)
         _MultiplicativeColor ("_MultiplicativeColor", Color) = (1,1,1,1)
...
//		Blend One OneMinusSrcAlpha // el original pero crea bordes raros como de textura recortada al aplicar la suma de _ColorSpecial
   ...
   //         half4 _ColorSpecial;
			...
			void Vert(posAndCoord IN, out posTwoCoordsAndHSVMults OUT)  
			{
				...
				OUT.color = IN.color; // aplico el color independiente del sprite IN.color
...
//                col.rgb = shift_col(col, half3(_HueShift, _Sat, _Val));
                ...
                col2.rgb += _AdditiveColor.rgb;
                col2.rgb *= _MultiplicativeColor.rgb;
