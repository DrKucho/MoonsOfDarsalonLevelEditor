...
public class LPShapeEditor: Editor	
{
	...
	public LPCorporeal _poly; // KUCHO HACK
	...
	public void DrawPointsUI(LPCorporeal poly)
    {
//		DrawDefaultInspector();
//		LPCorporeal poly = (LPCorporeal)target;	
	    _poly = poly; // KUCHO HACK
		string msg = "Draw Shape";	
		if (poly && poly.Drawing)
		{ 
			msg = "Stop Drawing";
		}
	...
	{
//		LPCorporeal poly = (LPCorporeal)target;	
        if (_poly && _poly.pointsList != null)// KUCHO HACK
		{
			if (_poly.Drawing)// KUCHO HACK (el guion bajo)
			{
				Drawpoints(_poly, e);	// KUCHO HACK (el guion bajo)
			}
			else if (e.shift)// KUCHO HACK (el guion bajo)
			{     	
				Movepoints(_poly, e);// KUCHO HACK (el guion bajo)
			}
			else if (e.control)// KUCHO HACK (el guion bajo)
			{     
				Removepoints(_poly, e);// KUCHO HACK (el guion bajo)
			}
			else if (e.alt)
			{     
				Insertpoints(_poly, e);// KUCHO HACK (el guion bajo)
			}
		}
    }
	...
	private bool DrawLines(Transform transform,List<Vector2> pointsList,Vector2 tran,Vector2 mousepos,int controlID,out int closest,out float dist
							,Color col, bool editme)
    {
//		tran = new Vector3(tran.x,tran.y,0f);
		Vector2[] points = LPShapeTools.TransformPoints(transform,tran,pointsList);
		closest = 0;
		dist = 10f;
...
//			points[j] = new Vector3(points[j].x,points[j].y,0f);
	...
	private bool CheckDists(Transform transform,List<Vector2> pointsList,Vector2 tran,Vector2 mousepos,int controlID
...
//			points[i] = new Vector3(points[i].x,points[i].y,0f);
		...
		for (int i = 0; i < points.Length; i++)
		{
			...
			Handles.DotHandleCap(controlID,points[i]+tran,Constants.zeroQ,HandleUtility.GetHandleSize(points[i]+tran)*0.07f, EventType.Repaint);
		}
		return isclosest;
	}
